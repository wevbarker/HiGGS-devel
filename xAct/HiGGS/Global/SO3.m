...
Get[FileNameJoin@{$HiGGSInstallDirectory,"Global","Smearing.m"}];


Get[FileNameJoin@{$HiGGSInstallDirectory,"Global","Induced.m"}];
...
(* ::Input::Initialization:: *)
(*
Options[TotalToO3]={"ToShell"\[Rule]True,"Order"\[Rule]Infinity};
...
res=res//ToNewCanonical;
res=ToO3[res,"ToShell"\[Rule]OptionValue["ToShell"],"Order"->OptionValue["Order"]];
res=ToOrderCanonical[res,OptionValue["Order"]];
NotebookDelete[printer];
res];
...
CDToD[x_,OptionsPattern[]]:=Module[{res,printer},
printer=PrintTemporary[" ** CDToD..."];
res=x;
If[OptionValue@UsexTensorCovD,
res//=xAct`HiGGS`Private`CDToGaugeCovD;res//=xAct`HiGGS`Private`GaugeCovDToLorentzGaugeCovD;res=res/.xAct`HiGGS`Private`ProjectLorentzGaugeCovDRule,res=res/.DGrandActivate;res=res/.DpGrandActivate];
...
Options[PreSimplify]={"Hard"\[Rule]False,"Order"\[Rule]Infinity};
...
Options[ToNesterForm]={"ToShell"\[Rule]True,"Hard"\[Rule]False,"Order"\[Rule]Infinity,"GToFoliG"\[Rule]True,xTensorCovD->False};
...
res=PreSimplify[res,"Hard"\[Rule]OptionValue["Hard"],"Order"\[Rule]OptionValue["Order"]];
res=TotalToO3[res,"ToShell"\[Rule]OptionValue["ToShell"],"Order"\[Rule]OptionValue["Order"]];
res=res~CDToD~(UsexTensorCovD->OptionValue@xTensorCovD);
res=TotalToO3[res,"ToShell"\[Rule]OptionValue["ToShell"],"Order"\[Rule]OptionValue["Order"]];
res=res//CDBToDJDV;
res=res~CDToD~(UsexTensorCovD->OptionValue@xTensorCovD);
res=TotalToO3[res,"ToShell"\[Rule]OptionValue["ToShell"],"Order"\[Rule]OptionValue["Order"]];
...
(* ::Input::Initialization:: *)
(*
Options[PoissonBracket]={"ToShell"\[Rule]True,"Hard"\[Rule]False,"Surficial"\[Rule]False,"Order"\[Rule]Infinity,"GToFoliG"\[Rule]True,"PreTruncate"\[Rule]False,"NesterForm"\[Rule]True,"PrintAnswer"->True,"Parallel"->False};
...
BarPartialf1B=ReplaceDummies@(Partialf1B-ReplaceIndex[Evaluate[Partialf1DBz],-q\[Rule]-w] A[w,-q,-z]);
BarPartialf2B=ReplaceDummies@(Partialf2B-ReplaceIndex[Evaluate[Partialf2DBz],-q\[Rule]-w] A[w,-q,-z]);
BarPartialf1A=ReplaceDummies@(Partialf1A-ReplaceIndex[Evaluate[Partialf1DAz],-q\[Rule]-w] A[w,-q,-z]-ReplaceIndex[Evaluate[Partialf1DAz],-r\[Rule]-w] A[w,-r,-z]);
BarPartialf2A=ReplaceDummies@(Partialf2A-ReplaceIndex[Evaluate[Partialf2DAz],-q\[Rule]-w] A[w,-q,-z]-ReplaceIndex[Evaluate[Partialf2DAz],-r\[Rule]-w] A[w,-r,-z]);
BarPartialf1BPi=ReplaceDummies@(Partialf1BPi+ReplaceIndex[Evaluate[Partialf1DBPiz],q\[Rule]w] A[q,-w,-z]);
BarPartialf2BPi=ReplaceDummies@(Partialf2BPi+ReplaceIndex[Evaluate[Partialf2DBPiz],q\[Rule]w] A[q,-w,-z]);
BarPartialf1APi=ReplaceDummies@(Partialf1APi+ReplaceIndex[Evaluate[Partialf1DAPiz],q\[Rule]w] A[q,-w,-z]+ReplaceIndex[Evaluate[Partialf1DAPiz],r\[Rule]w] A[r,-w,-z]);
BarPartialf2APi=ReplaceDummies@(Partialf2APi+ReplaceIndex[Evaluate[Partialf2DAPiz],q\[Rule]w] A[q,-w,-z]+ReplaceIndex[Evaluate[Partialf2DAPiz],r\[Rule]w] A[r,-w,-z]);
...
2ReplaceIndex[Evaluate[Partialf1DAz],z\[Rule]w] ManualCovariantDerivative[-w,Partialf2DAPiz,IndexList[z,-s],u]);
...
Get[FileNameJoin@{$HiGGSInstallDirectory,"Global","PoissonBracket.m"}];
